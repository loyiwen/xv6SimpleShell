...
__attribute__((noreturn)) void run_command(char *buf, int nbuf, int *pcp) {
	...
	if (redirection_left) {
		if (file_name_l) {
			if (file_name_l == 0 || strlen(file_name_l) == 0) {
				fprintf(2, "missing file for redirection\n");
        		exit(1);
			}
			...
			close(0); // Close stdin
			if (dup(fd_in) != 0) {
				fprintf(2, "dup failed for input redirection\n");
				close(fd_in);
				exit(1);
			}
			close(fd_in);
	...
	if (redirection_right) {
		if (file_name_r) {
			if (file_name_r == 0 || strlen(file_name_r) == 0) {
				fprintf(2, "missing file for redirection\n");
        		exit(1);
			}
			...
			if (dup(fd_out) != 1) {
				fprintf(2, "dup failed for output redirection\n");
				close(fd_out);
				exit(1);
			}
			close(fd_out);
